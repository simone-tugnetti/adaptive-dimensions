...
import it.simonetugnetti.adaptivedimensions.compose.utils.heightAdaptiveSp
import it.simonetugnetti.adaptivedimensions.compose.utils.maxHeightAdaptiveSp
 ...
 *
 * ```
 * CompositionLocalProvider(LocalStaticAsp provides AdaptiveSp.SW300.asp) {
 *      LocalStaticAsp.current._1asp // = 1.sp
 *      Composable(LocalAsp.current._1asp) // Re-compose
 *      Composable() // Re-compose
 *      CompositionLocalProvider(LocalStaticAsp provides AdaptiveSp.SW600.asp) {
 *          LocalStaticAsp.current._1asp // = 2.sp
 *      }
 * }
 * ```
 *
...
/**
 * Retrieve the current value of [LocalStaticAsp] called directly
 * in [MaterialTheme].
 *
 * ```
 * MaterialTheme.staticAsp._1asp // = LocalStaticAsp.current._1asp
 * ```
 *
 * @since 1.0.0
 * @see LocalStaticAsp
 * @see AdaptiveSp
 */
val MaterialTheme.staticAsp: Asp
    @Composable
    @ReadOnlyComposable
    get() = LocalStaticAsp.current
 ...
 * @see AdaptiveSp
 */
val Material3Theme.staticAsp: Asp
    @Composable
    @ReadOnlyComposable
    get() = LocalStaticAsp.current
...
/**
 * Update [LocalStaticAsp] key by the given `adaptiveSp`.
 *
 * @since 1.0.0
 * @param adaptiveSp An [AdaptiveSp] entry
 * @param content Composable inside [CompositionLocalProvider]
 * @see CompositionLocalProviderAdaptiveSp
 */
@Composable
@NonSkippableComposable
fun CompositionLocalProviderStaticAdaptiveSp(
    adaptiveSp: AdaptiveSp,
    content: @Composable () -> Unit
) {
    CompositionLocalProvider(LocalStaticAsp provides adaptiveSp.asp) {
        content()
    }
}
...
/**
 * Update [LocalStaticAsp] key by the given [WindowSizeClass].
 *
...
@Composable
@NonSkippableComposable
fun CompositionLocalProviderStaticAdaptiveSpByWindowSizeClass(
    ...
    CompositionLocalProviderStaticAdaptiveSp(
        if (isPortrait) windowSizeClass.windowWidthSizeClass.adaptiveSp
        else windowSizeClass.windowHeightSizeClass.adaptiveSp
    ) {
        content()
    }
...
fun CompositionLocalProviderStaticAdaptiveSpByWindowSizeClass(
    ...
    CompositionLocalProviderStaticAdaptiveSp(
        if (isPortrait) windowWidthSizeClass.adaptiveSp
        else windowHeightSizeClass.adaptiveSp
    ) {
...
/**
 * Update [LocalStaticAsp] key by the given [IntSize].
 *
 * @since 1.0.0
...
@Composable
@NonSkippableComposable
fun CompositionLocalProviderStaticAdaptiveSpByWindowSize(
    ...
    CompositionLocalProviderStaticAdaptiveSp(
        if (isPortrait) windowSize.widthAdaptiveSp(resources)
        else windowSize.heightAdaptiveSp(resources)
    ) {
        content()
    }
...
/**
 * Update [LocalStaticAsp] key by the given [DpSize].
 *
 * @since 1.0.0
...
@Composable
@NonSkippableComposable
fun CompositionLocalProviderStaticAdaptiveSpByWindowDpSize(
    ...
    CompositionLocalProviderStaticAdaptiveSp(
        if (isPortrait) windowDpSize.widthAdaptiveSp
        else windowDpSize.heightAdaptiveSp
    ) {
        content()
    }
 ...
 * @see BoxWithConstraintsAdaptiveSp
...
@Composable
@UiComposable
fun BoxWithConstraintsStaticAdaptiveSp(
    ...
    BoxWithConstraints(
        ...
        CompositionLocalProviderStaticAdaptiveSp(
            if (isPortrait) maxWidthAdaptiveSp
            else maxHeightAdaptiveSp
        ) {
            content()
        }
